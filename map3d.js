(async () => {
  ...
  const centerLon = -3.65;
  const centerLat = 40.05;
  const scale = 0.62;
  const toPoint = (lon, lat, z = 0) => new THREE.Vector3((lon - centerLon) * scale, (lat - centerLat) * scale, z);

  const mainland = [
    [-9.28, 43.18], [-8.84, 42.52], [-8.92, 41.92], [-8.78, 41.18],
    [-8.98, 40.52], [-9.12, 39.72], [-9.20, 38.72], [-8.90, 37.28],
    [-8.42, 36.95], [-7.44, 37.05], [-6.55, 36.20], [-5.62, 36.02],
    [-4.70, 36.10], [-3.36, 36.70], [-2.10, 36.78], [-1.18, 37.35],
    [-0.28, 38.18], [0.28, 39.42], [0.84, 40.48], [2.02, 41.16],
    [3.08, 41.86], [2.72, 42.34], [1.58, 42.72], [0.18, 42.68],
    [-1.44, 43.28], [-3.18, 43.48], [-4.46, 43.58], [-5.80, 43.64],
    [-7.16, 43.62], [-8.30, 43.48]
  ];

  const portugalBorder = [
    [-7.05, 42.10], [-6.72, 41.66], [-6.92, 41.08], [-7.05, 40.36],
    [-6.86, 39.68], [-7.28, 39.02], [-7.12, 38.44], [-7.44, 37.72],
    [-7.40, 37.18]
  ];
  ...
  const outline = new THREE.Line(
    ...
    new THREE.LineBasicMaterial({ color: 0x36b8ff, transparent: true, opacity: 0.98 })
  );
  root.add(outline);

  const portugalLine = new THREE.Line(
    new THREE.BufferGeometry().setFromPoints(portugalBorder.map(([lon, lat]) => toPoint(lon, lat, 0.12))),
    new THREE.LineBasicMaterial({ color: 0x36b8ff, transparent: true, opacity: 0.58 })
  );
  root.add(portugalLine);

  const coastGlow = new THREE.Line(
    new THREE.BufferGeometry().setFromPoints(outlinePoints.map((point) => point.clone().setZ(0.14))),
    new THREE.LineBasicMaterial({ color: 0x2f7dff, transparent: true, opacity: 0.28 })
  );
  coastGlow.scale.setScalar(1.012);
  root.add(coastGlow);
  ...
  const resize = () => {
    ...
    if (width < 620) {
      camera.position.set(0.1, -8.5, 7.05);
      root.position.set(0.0, -0.06, 0);
      root.scale.setScalar(0.82);
    } else {
      camera.position.set(0.1, -8.05, 6.35);
      root.position.set(0.0, -0.08, 0);
